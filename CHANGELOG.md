...
- **Settings window** — popover top-right gear opens a dedicated Settings `Window` (`id: "settings"`) with a sidebar: General (language + Open at Login), Keys (keygen), Import (auto-detects existing config), Backups (no duplicate title / Done chrome), Config (Remove Include). Popover header is **KeyChord** + gear on one row (no separate Accounts section title). Accounts toolbar keeps only **Add account**.

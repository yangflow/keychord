...
- **Backup policy** — `AccountsStore` snapshots `accounts.json` only when **adding** an account (if a file already exists). `update` / `delete` / `touchLastUsed` / `replaceAll` write without a new backup. Settings → Backups can delete a snapshot; rows use rounded cards (ScrollView stack) instead of a Form list.

...
- **Menu bar icon folder drop** — drag a folder onto the KeyChord menu bar icon (popover need not stay open). Resolves via `CurrentRepoResolver`, stores the match on `AppState`, and opens the popover. Idle copy no longer claims you can drop into the dashed box. **Choose Folder…** presents an `NSOpenPanel` sheet on the popover so the match appears in place (no close/reopen). The match is one-shot: dismissing the popover (or the card’s clear control) clears it; silent Finder auto-resolve on open is gone. Matched repos with `origin` prefill the compact clone field as `owner/repo` so copy is enabled immediately.

...
- **Stale `gitdir:` paths can be retargeted** — drop a renamed or moved project and, when an account still points at a **missing** folder that looks like its previous name (same parent directory, or the same folder name under a new parent, with the account label breaking ties), the card says the old path is gone, names the account, and offers **Point it at this folder** (replaces just that one path) or **Keep the old path**. Paths that still exist are never called stale and no other account is touched. (#38)
...
- **Match-card conveniences** — a 5-second **Bound to {label} · Undo** toast after a bind or rebind (undo restores the previous gitdir lists verbatim); clicking the path copies it; an HTTPS remote offers **Copy set-url** for `git remote set-url origin git@<alias>:owner/repo.git` (host and transport only — the path, subgroups included, is carried over and ends in `.git` like the clone helper's output); and the clone field remembers the last `org/` prefix per account, on the match card and the per-account clone row alike. (#42)
- **Most recently used identity first** — the popover list (and the Accounts sidebar) sort by `lastUsedAt` descending, then label, with never-used accounts last. Matching, binding, copying a clone command, and opening an account all count as use. The #35 filter applies after the sort, so typing never reshuffles the list. (#43)
- **Doctor collapses when everything is green** — a healthy machine gets one **Configuration looks good** line instead of a tall block; a new set of findings expands itself, and re-running with the same findings respects a manual collapse. Fixer actions are untouched when there are issues. (#44)
